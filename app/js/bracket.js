 ...
 */
 ...
 $(document).ready(function(){
  ...
  $('#btnLeftMenuMobile').on('click', function(e){
    /*$('body').addClass('show-left');*/
    $('.br-logo-mobile').animate({
      opacity : 0
    }, {
      duration: 400,
      specialEasing: {
  ...
  // handles .br-logo-mobile change states on window resize
  $(window).on('resize', function() {
    var win = $(this); // this window
    if (win.height() <= 991.98) {
      if (!$('body').hasClass("show-left") && !$('body').hasClass("collapsed-menu")) {
        $('.br-logo-mobile').animate({opacity: 1}, "fast");
      }
      if ($('body').hasClass("show-left") && !$('body').hasClass("collapsed-menu")) {
        $('.br-logo-mobile').animate({opacity: 0}, "fast");
      }
      if (!$('body').hasClass("show-left") && $('body').hasClass("collapsed-menu")) {
        $('.br-logo-mobile').animate({opacity: 1}, "fast");
      }
  ...
  $(document).on('click touchstart', function(e){
    ...
    if($('body').hasClass('show-left')) {
      var targ = $(e.target).closest('.br-sideleft').length;
      if(!targ) {
        $('body').removeClass('show-left').find('.br-logo-mobile').delay(400).animate({opacity : 1}, "fast");
      }
    }
